...
- (void)AliyunUploadWithFileData:(NSData *)data
                   ...
                   completeBlock:(UPCompeleteBlock)completeBlock
                        progress:(UPProgressBlock)upProgress {
    
    OSSPutObjectRequest * put = [OSSPutObjectRequest new];
    
    // required fields
    put.bucketName = [UPYUNConfig sharedInstance].AliyunBucket;
...
//        NSLog(@"%lld, %lld, %lld", bytesSent, totalByteSent, totalBytesExpectedToSend);
        dispatch_async(dispatch_get_main_queue(), ^(){
            if (upProgress) {
                upProgress(1.0*totalByteSent/totalBytesExpectedToSend, totalByteSent);
            }
        });
    };
    // 可选参数
...
//                        @"<var2>": @"<value2>"};
    ...
    [putTask continueWithBlock:^id(OSSTask *task) {
        ...
        NSHTTPURLResponse * response = [[NSHTTPURLResponse alloc] initWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://%@.oss-cn-hangzhou.aliyuncs.com/", [UPYUNConfig sharedInstance].AliyunBucket]] statusCode:result.httpResponseCode HTTPVersion:@"1.1" headerFields:result.httpResponseHeaderFields];
        
        NSDictionary *dic = @{@"response":response, @"responseData":string};
        dispatch_async(dispatch_get_main_queue(), ^(){
            if (completeBlock) {
                completeBlock(task.error, dic, task.error==nil);
            }
        });
        return nil;
    }];
...
- (void)AliyunMutUploadWithFileData:(NSData *)data
    ...
    _uploadToBucket = [UPYUNConfig sharedInstance].AliyunBucket;
    _uploadObjectkey = savekey;
    ...
    NSInteger blockCount = ceil(_fileLength*1.0/[UPYUNConfig sharedInstance].QiniuBlockSize);
    ...
    [initTask continueWithBlock:^id(OSSTask *task) {
        if (!task.error) {
            ...
            for (int i = 1; i <= blockCount; i++) {
                [weakSelf uploadBlock:i progressBlock:progressBlock completeBlock:completeBlock];
            }
        } else {
...
- (void)uploadBlock:(int32_t)index progressBlock:(UPProgressBlock)progressBlock completeBlock:(UPCompeleteBlock)completeBlock{
    
    
    NSData *data;
    if (_filePath) {
        data = [self readChunkWithFilePath:_filePath offset:[UPYUNConfig sharedInstance].AlinyunBlockSize * (index - 1)];
    } else {
        data = [self readChunkWithFileData:_fileData Offset:[UPYUNConfig sharedInstance].AlinyunBlockSize * (index - 1)];
    }
    ...
    [uploadPartTask continueWithBlock:^id(OSSTask *task) {
        if (!task.error) {
            ...
            if (![_partInfos containsObject:[NSNull null]]) {
                [weakSelf makeFileCompleteBlock:completeBlock];
            }
...
- (void)makeFileCompleteBlock:(UPCompeleteBlock)completeBlock {
...
- (void)initOSSClient {
    ...
    id<OSSCredentialProvider> credential = [[OSSPlainTextAKSKPairCredentialProvider alloc] initWithPlainTextAccessKey:[UPYUNConfig sharedInstance].AliyunAccessKey secretKey:[UPYUNConfig sharedInstance].AliyunSecretKey];
...
//    }];
    ...
    client = [[OSSClient alloc] initWithEndpoint:[UPYUNConfig sharedInstance].AliyunEndPoint credentialProvider:credential clientConfiguration:conf];
}

- (NSData *)readChunkWithFileData:(NSData *)fileData Offset:(NSInteger)offset {
    
    NSInteger length = [UPYUNConfig sharedInstance].AlinyunBlockSize;
...
- (NSData *)readChunkWithFilePath:(NSString *)filePath offset:(NSInteger)offset {
    ...
    NSData *subData = [handle readDataOfLength:[UPYUNConfig sharedInstance].AlinyunBlockSize];
    [handle closeFile];
    return [subData copy];
}
