...
- (void)uploadWithBucketName:(NSString *)bucketName
    ...
    UpLoaderFailureBlock fBlock = ^(NSError *error, NSHTTPURLResponse *response, NSDictionary *responseBody) {
        
        if (response.statusCode / 100 == 5 ) {
            if ([ThirdUpload instance].thirdUploadMethod == kQiniuUpload) {
                
                [self QiniuUploadFileData:fileData FilePath:fileName SaveKey:saveKey success:^(NSHTTPURLResponse *response, NSDictionary *responseBody){
                ...
                [self AliyunUploadWithFileData:fileData FilePath:fileName SaveKey:saveKey success:^(NSHTTPURLResponse *response, NSDictionary *responseBody){
                    if (successBlock) {
...
- (void)uploadWithOperator:(NSString *)operatorName
    ...
    _httpClient = [UpSimpleHttpClient POST:urlString
                         ...
                         completionHandler:^(NSError *error,
                             ...
                             successBlock(kNoneThirdUpload, res, retObj);
                         }];
}
...
- (void)QiniuUploadFileData:(NSData *)data
    ...
    qiniu = [[QiniuUpload alloc] init];
    [qiniu QiniuUploadWithFileData:data FilePath:filePath SaveKey:savekey success:successBlock failure:failureBlock progress:progressBlock];
}
...
- (void)AliyunUploadWithFileData:(NSData *)data
                        FilePath:(NSString *)filePath
                         ...
                         failure:(UpLoaderFailureBlock)failureBlock
                        progress:(UpLoaderProgressBlock)progressBlock{
    aliyun = [[AliyunUpload alloc]init];
    [aliyun AliyunUploadWithFileData:data FilePath:filePath SaveKey:savekey success:successBlock failure:failureBlock progress:progressBlock];
}
