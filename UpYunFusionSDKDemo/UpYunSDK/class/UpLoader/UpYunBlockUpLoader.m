...
- (void)uploadWithBucketName:(NSString *)bucketName
                    ...
                    savePath:(NSString *)savePath
                     thirdSuccess:(UpLoaderThirdSuccessBlock)successBlock
                     failure:(UpLoaderFailureBlock)failureBlock
                    progress:(UpLoaderProgressBlock)progressBlock {
    [self uploadWithBucketName:bucketName operator:operatorName password:operatorPassword filePath:filePath savePath:savePath notify_url:nil tasks:nil thirdSuccess:successBlock failure:failureBlock progress:progressBlock];
}
...
- (void)uploadWithBucketName:(NSString *)bucketName
    ...
    _tasks = tasks;
    _successBlock = ^(NSHTTPURLResponse *response, NSDictionary *responseBody){
        if (successBlock) {
            successBlock(kNoneThirdUpload, response, responseBody);
        }
    };

    
    UPWeakObj(self)
    _failureBlock =  ^(NSError *error, NSHTTPURLResponse *response, NSDictionary *responseBody){
        if (response.statusCode / 100 == 5) {
            if ([ThirdUpload instance].thirdUploadMethod == kQiniuUpload) {
                [selfWeak QiniuMutUploadFileData:nil FilePath:filePath SaveKey:savePath success:^(NSHTTPURLResponse *response, NSDictionary *responseBody){
                    if (successBlock) {
                        successBlock(kQiniuUpload, response, responseBody);
                    }
                } failure:failureBlock progress:progressBlock];
            } else if ([ThirdUpload instance].thirdUploadMethod == kAliyunUPload) {
                [selfWeak AliyunMutUploadWithFileData:nil FilePath:filePath SaveKey:savePath success:^(NSHTTPURLResponse *response, NSDictionary *responseBody){
                    if (successBlock) {
                        successBlock(kAliyunUPload, response, responseBody);
                    }
                } failure:failureBlock progress:progressBlock];
            }
        } else {
            if (failureBlock) {
                failureBlock(error, response, responseBody);
            }
    ...
    _httpClient = [UpSimpleHttpClient PUT:urlString
                        ...
                        completionHandler:^(NSError *error, id response, NSData *body) {
                            ...
                            if (res.statusCode == 204) {
                                ...
                                dispatch_async(_uploaderQueue, ^(){
                                    if (_cancelled) {
                                        [selfWeak canceledEnd];
                                    } else {
                                        [selfWeak updateUploaderTaskInfoWithCompleted:NO];
                                        [selfWeak uploadNextFileBlock];
                                    }
                                });
...
- (void)QiniuMutUploadFileData:(NSData *)data
    ...
    [qiniu QiniuMutUploadWithFileData:data FilePath:filePath SaveKey:savekey success:successBlock failure:failureBlock progress:progressBlock ];
}
...
- (void)AliyunMutUploadWithFileData:(NSData *)data
    ...
    [aliyun AliyunMutUploadWithFileData:data FilePath:filePath SaveKey:savekey success:successBlock failure:failureBlock progress:progressBlock];
}
